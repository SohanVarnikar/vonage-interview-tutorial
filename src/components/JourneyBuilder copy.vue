<template>
  <div class="flex flex-col h-screen font-sans">
    <div class="bg-gray-100 border-b border-gray-300 p-4">
      <h2 class="text-2xl font-medium text-gray-700 m-0 mb-4">Journey Builder Dashboard</h2>
      <div class="flex items-center justify-between gap-4 flex-wrap">
        <div class="flex flex-wrap gap-3">
          <button 
            ...
            @click="addNode(nodeType.type)">
              <div class="flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 group-hover:from-gray-200 group-hover:to-gray-300 transition-all duration-300 group-hover:scale-110">
                <!-- Start Icon -->
                <svg v-if="nodeType.type === 'start'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-4 h-4 text-gray-700 transition-all duration-300 group-hover:scale-110" aria-hidden="true">
                  <polygon points="5,3 19,12 5,21" fill="currentColor" />
                </svg>
                <!-- Email Icon -->
                <svg v-else-if="nodeType.type === 'email'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-4 h-4 text-gray-700 transition-all duration-300 group-hover:scale-110" aria-hidden="true">
                  <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
                  <polyline points="22,6 12,13 2,6" />
                </svg>
                <!-- Wait/Clock Icon -->
                <svg v-else-if="nodeType.type === 'wait'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-4 h-4 text-gray-700 transition-all duration-300 group-hover:scale-110" aria-hidden="true">
                  <circle cx="12" cy="12" r="10" />
                  <polyline points="12,6 12,12 16,14" />
                </svg>
                <!-- Condition/Branch Icon -->
                <svg v-else-if="nodeType.type === 'condition'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-4 h-4 text-gray-700 transition-all duration-300 group-hover:scale-110" aria-hidden="true">
                  <path d="M6 3l6 6-6 6" />
                  <path d="M12 3l6 6-6 6" />
                </svg>
                <!-- Input Icon -->
                <svg v-else-if="nodeType.type === 'inputtext'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-4 h-4 text-gray-700 transition-all duration-300 group-hover:scale-110" aria-hidden="true">
                  <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
                  <line x1="7" y1="12" x2="17" y2="12" />
                </svg>
                <!-- Output Icon -->
                <svg v-else-if="nodeType.type === 'outputtext'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-4 h-4 text-gray-700 transition-all duration-300 group-hover:scale-110" aria-hidden="true">
                  <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
                ...
                <!-- End/Check Icon -->
                <svg v-else-if="nodeType.type === 'end'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-4 h-4 text-gray-700 transition-all duration-300 group-hover:scale-110" aria-hidden="true">
                  <path d="M9 11l3 3L22 4" />
                  <path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11" />
                </svg>
              </div>
              <span class="font-semibold text-sm text-gray-800">{{ nodeType.label }}</span>
        ...
        <div class="flex justify-end">
          <button @click="saveJourney" class="group relative px-6 py-3 border-2 border-blue-500 rounded-xl cursor-pointer font-bold text-white bg-blue-500 hover:bg-blue-600 hover:border-blue-600 transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-105 active:scale-100 focus:outline-none focus:ring-4 focus:ring-blue-300 focus:ring-opacity-50">
            <div class="flex items-center gap-2">
              <div class="flex items-center justify-center w-6 h-6 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 group-hover:from-blue-500 group-hover:to-blue-700 transition-all duration-300 group-hover:scale-110">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-3 h-3 transition-all duration-300 group-hover:scale-110" aria-hidden="true">
    ...
    <div class="flex-1 relative bg-gray-50">
      ...
      >
        ...
        <template #node-start="{ data, id }">
          <StartNode :data="data" :id="id" @remove="removeNode" @update="handleNodeUpdate(id, $event)" />
        </template>
        
        <template #node-email="{ data, id }">
          <EmailNode :data="data" :id="id" @remove="removeNode" @update="handleNodeUpdate(id, $event)" />
        </template>
        
        <template #node-wait="{ data, id }">
          <WaitNode :data="data" :id="id" @remove="removeNode" @update="handleNodeUpdate(id, $event)" />
        </template>
        
        <template #node-condition="{ data, id }">
          <ConditionNode :data="data" :id="id" @remove="removeNode" @update="handleNodeUpdate(id, $event)" />
        </template>
        
        <template #node-inputtext="{ data, id }">
          <InputNode :data="data" :id="id" @remove="removeNode" @update="handleNodeUpdate(id, $event)" />
        </template>
        
        <template #node-outputtext="{ data, id }">
          <OutputNode :data="data" :id="id" @remove="removeNode" @update="handleNodeUpdate(id, $event)" />
        </template>
    ...
    <div v-if="selectedNode" class="absolute top-20 right-5 w-80 bg-white border border-gray-200 rounded-lg p-4 shadow-lg">
      <h3 class="text-lg text-gray-700 mb-4 m-0">Properties</h3>
      <div class="mb-4">
        <label class="block mb-1 font-medium text-gray-700">Node Name:</label>
        <input v-model="selectedNode.data.label" @input="updateNode" class="w-full p-2 border border-gray-200 rounded text-sm" />
      </div>
      <div v-if="selectedNode.type === 'email'" class="mb-4">
        <label class="block mb-1 font-medium text-gray-700">Subject:</label>
        <input v-model="selectedNode.data.subject" @input="updateNode" class="w-full p-2 border border-gray-200 rounded text-sm mb-4" />
        <label class="block mb-1 font-medium text-gray-700">Template:</label>
        <textarea v-model="selectedNode.data.template" @input="updateNode" class="w-full p-2 border border-gray-200 rounded text-sm min-h-20 resize-y"></textarea>
      </div>
      <div v-if="selectedNode.type === 'wait'" class="mb-4">
        <label class="block mb-1 font-medium text-gray-700">Duration:</label>
        <input v-model="selectedNode.data.duration" @input="updateNode" class="w-full p-2 border border-gray-200 rounded text-sm mb-4" />
        <select v-model="selectedNode.data.unit" @change="updateNode" class="w-full p-2 border border-gray-200 rounded text-sm">
      ...
      <div v-if="selectedNode.type === 'condition'" class="mb-4">
        ...
        <select v-model="selectedNode.data.condition" @change="updateNode" class="w-full p-2 border border-gray-200 rounded text-sm">
...
const { findNode, addNodes, addEdges, removeNodes, removeEdges, project } = useVueFlow()
...
const removeAutoEdgesForNode = (nodeId) => {
  const autoEdges = elements.value.filter(el => 
    el.source && 
...
const createAutoConnection = (sourceId, targetId, variableName) => {
  ...
  if (!existingEdge) {
    ...
    addEdges([autoEdge])
    console.log(`Auto-connected: ${sourceId} → ${targetId} (variable: ${variableName})`)
  }
}
...
const getDefaultNodeData = (nodeType) => {
  const baseData = { label: `${nodeType.charAt(0).toUpperCase() + nodeType.slice(1)} Node` }
...
const onNodeClick = ({ node }) => {
  ...
  if (node.type === 'start' || node.type === 'email' || node.type === 'inputtext' || node.type === 'outputtext') {
    return
  }
...
const validateConnections = () => {
  ...
  if (nodes.length === 0) {
    return { isValid: true, message: 'No nodes to validate' }
  }
  
  if (nodes.length === 1) {
    return { isValid: true, message: 'Single node journey is valid' }
  }
  ...
  nodes.forEach(node => {
    ...
    if (node.type === 'start') {
      if (!hasOutgoing) {
        issues.push(`Start node "${node.data.label || node.id}" has no outgoing connections`)
        disconnectedNodes.push(node.id)
      }
    } else if (node.type === 'end') {
      if (!hasIncoming) {
        issues.push(`End node "${node.data.label || node.id}" has no incoming connections`)
        disconnectedNodes.push(node.id)
      }
    } else {
      // For middle nodes (email, wait, condition, input, output)
      if (!hasIncoming && !hasOutgoing) {
        issues.push(`Node "${node.data.label || node.id}" is completely isolated`)
        disconnectedNodes.push(node.id)
      } else if (!hasIncoming) {
        issues.push(`Node "${node.data.label || node.id}" has no incoming connections`)
        disconnectedNodes.push(node.id)
      } else if (!hasOutgoing) {
        issues.push(`Node "${node.data.label || node.id}" has no outgoing connections`)
      }
    }
  })
  ...
  if (issues.length > 0) {
    return {
      isValid: false,
      message: `Connection issues found:\n\n${issues.join('\n')}`,
      disconnectedNodes
    }
  }
  
  return { isValid: true, message: 'All nodes are properly connected' }
}
...
const validateFormFields = () => {
  ...
  for (const node of nodes) {
    ...
    // Check for empty name/label
    if (!nodeData.label || nodeData.label.trim() === '') {
      errors.push(`${node.type.charAt(0).toUpperCase() + node.type.slice(1)} node is missing a name`)
    }
    ...
    switch (node.type) {
      case 'inputtext':
        if (!nodeData.name || nodeData.name.trim() === '') {
          errors.push(`Input node "${nodeData.label || 'Unnamed'}" is missing input name`)
        }
        break
      
      case 'outputtext':
        if (!nodeData.message || nodeData.message.trim() === '') {
          errors.push(`Output node "${nodeData.label || 'Unnamed'}" is missing message text`)
        }
        break
      
      case 'email':
        if (!nodeData.subject || nodeData.subject.trim() === '') {
          errors.push(`Email node "${nodeData.label || 'Unnamed'}" is missing subject`)
        }
        if (!nodeData.template || nodeData.template.trim() === '') {
          errors.push(`Email node "${nodeData.label || 'Unnamed'}" is missing template`)
        }
        break
      
      case 'wait':
        if (!nodeData.duration || nodeData.duration <= 0) {
          errors.push(`Wait node "${nodeData.label || 'Unnamed'}" is missing duration`)
        }
        break
      
      case 'condition':
        if (!nodeData.condition || nodeData.condition.trim() === '') {
          errors.push(`Condition node "${nodeData.label || 'Unnamed'}" is missing condition`)
        }
        break
    }
  }
...
const postJourneyData = async (journeyData) => {
  try {
    ...
    console.error('Error posting journey data:', error)
...
const saveJourney = async () => {
  ...
  if (!fieldValidation.isValid) {
    showAlert(
      'error',
      'Cannot Save Journey',
      `Please fix the following issues:<br>• ${fieldValidation.errors.join('<br>• ')}`,
      { autoClose: false }
    )
    return
  }
  ...
  showAlert(
    'info',
    'Saving Journey',
    'Please wait while we save your journey...'
  )
  
  // Post to API
  const apiResult = await postJourneyData(journeyData)
  ...
  if (apiResult.success) {
    ...
    showAlert(
      ...
      `Your journey has been saved!<br>API Response ID: ${apiResult.data.id}`
    )
    ...
    showAlert(
      'error',
      'Save Failed',
      `Failed to save journey to server: ${apiResult.error}<br>Data saved locally as backup.`,
      { autoClose: false }
    )
...
const initializeSampleFlow = () => {
  const sampleNodes = [
    ...
    },
    {
      ...
      data: { label: 'Welcome Email', subject: 'Welcome!', template: 'Welcome to our platform!' }
    },
    {
      id: 'wait-1',
      type: 'wait',

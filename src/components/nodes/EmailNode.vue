<template>
  <div class="group relative">
    <div class="px-0 py-0 rounded-2xl min-w-64 max-w-96 shadow-xl cursor-pointer transition-all duration-300 bg-white border-2 border-blue-500 text-blue-800 hover:shadow-2xl focus:outline-none" style="width: auto;">
      <!-- Header -->
      ...
      <div class="px-4 py-3">
        <div class="space-y-4">
          ...
          <div class="text-blue-600 text-sm mb-3">Send message</div>
          <div class="bg-gray-100 rounded-lg p-3">
            <textarea 
              v-model="contentValue"
              @input="updateContent"
              placeholder="Email content..."
              class="w-full text-sm text-blue-700 bg-transparent resize-none focus:outline-none leading-relaxed"
              rows="3"
...
const updateContent = () => {
  emit('update', { ...props.data, content: contentValue.value })
  // Auto-resize textarea based on content
  nextTick(() => {
    const textarea = document.querySelector(`[data-node-id="${props.id}"] textarea`)
    if (textarea) {
